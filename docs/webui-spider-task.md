...
| 分类名 | `行业研报/个股研报/宏观研究/策略报告/券商晨报`——**与 reader 的 5 个 CATEGORIES 完全一致（已实测验证）**                                      |
| 测试隔离 | `--test` 模式输出到 `report_data/test/{date}/`,**与全量数据隔离**（不会污染 reader 读到的正式数据）                                      |
...
| type     | 命令(cwd)                                                                                                                                                                        |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `spider` | `[SPIDER_PYTHON, "-m", "src.industry_report_spider.industry_report_spider", "--type", rt, "--start", s, "--end", e, (+ "--test"), (+ "--limit" n)]`,**cwd = `D:\WORKS\all_data`** |

> 参数命名:params 里的研报类型键叫 `report_type`（**不要叫 `type`**,与任务类型字段冲突）。
...
| 期           | 内容                                                                                                            | 自检                                                                                             |
| ------------ | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **v1**(本轮) | spider 单任务:抓取 → 日志流 → 完成提示                                                                          | 页面发起 `--test --limit 2` 抓行业研报 → 日志滚动 → `report_data/test/{今天}/行业研报/` 出现 2 个 txt |
...
| AC-4 | 数据落地 | 测试模式结束后 `report_data/test/{date}/` 出现对应分类 txt；全量模式才写入 `report_data/{date}/`（可被 reader 消费） |
